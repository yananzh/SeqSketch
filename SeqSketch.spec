...
# EXE options that only make sense on one platform.
#   Windows: version resource, application manifest and .ico icon.
#   macOS:   an .icns icon (generated by scripts/make_icns.py); it rejects the
#            .ico and the other two options.
_mac_icon = os.path.join(root, 'window_logo.icns')
_platform_exe_options = {}
if sys.platform.startswith('win'):
    _platform_exe_options = {
...
elif os.path.exists(_mac_icon):
    _platform_exe_options = {'icon': _mac_icon}
else:
    print('WARNING: window_logo.icns is missing - the macOS app gets the '
          'default PyInstaller icon. Run: py scripts/make_icns.py')
...
if sys.platform == 'darwin':
    app = BUNDLE(
        ...
        # BUNDLE does NOT inherit the EXE icon: without this it silently falls
        # back to PyInstaller's generic icon-windowed.icns. PyInstaller fills in
        # CFBundleIconFile from this path itself.
