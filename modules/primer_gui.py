...
class Worker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            # design_primers 是 primer3-py 库的核心函数
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.results_table.setHorizontalHeaderLabels([
            "Pair #", "Fwd Primer", "Fwd Len", "Fwd Tm", "Fwd GC%", 
            "Rev Primer", "Rev Len", "Rev Tm", "Rev GC%", "Product Size"
        ])
    ...
    def start_design_task(self):
        ...
        # 1. 准备序列参数 (SEQUENCE_ID, SEQUENCE_TEMPLATE)
        seq_args = {
        ...
        if self.rb_specific.isChecked():
            ...
            included_region = (start_pos - 1, end_pos - start_pos + 1)
            ...
            # 关键参数: 强制目标为整个序列
            seq_args['SEQUENCE_TARGET'] = (0, seq_len)
            global_args['PRIMER_PRODUCT_SIZE_RANGE'] = [
    ...
    def update_results_table(self, results):
        ...
        self.results_table.setRowCount(num_returned)
        ...
        for i in range(num_returned):
            ...
            fwd_len = results[f'PRIMER_LEFT_{i}']['LENGTH']
            fwd_tm = results[f'PRIMER_LEFT_{i}']['TM']
            fwd_gc = results[f'PRIMER_LEFT_{i}']['GC_PERCENT']
            rev_len = results[f'PRIMER_RIGHT_{i}']['LENGTH']
            rev_tm = results[f'PRIMER_RIGHT_{i}']['TM']
            rev_gc = results[f'PRIMER_RIGHT_{i}']['GC_PERCENT']
            
            # 存储详细信息以备后用
            pair_info = results[f'PRIMER_PAIR_{i}']
            pair_info['LEFT'] = results[f'PRIMER_LEFT_{i}']
            pair_info['RIGHT'] = results[f'PRIMER_RIGHT_{i}']
            self.primer_pair_details.append(pair_info)

            # 填充表格
            self.results_table.setItem(i, 0, QTableWidgetItem(str(i + 1)))
            self.results_table.setItem(i, 1, QTableWidgetItem(fwd_seq))
            self.results_table.setItem(i, 2, QTableWidgetItem(str(fwd_len)))
            self.results_table.setItem(i, 3, QTableWidgetItem(f"{fwd_tm:.2f}"))
            self.results_table.setItem(i, 4, QTableWidgetItem(f"{fwd_gc:.2f}"))
            self.results_table.setItem(i, 5, QTableWidgetItem(rev_seq))
            self.results_table.setItem(i, 6, QTableWidgetItem(str(rev_len)))
            self.results_table.setItem(i, 7, QTableWidgetItem(f"{rev_tm:.2f}"))
            self.results_table.setItem(i, 8, QTableWidgetItem(f"{rev_gc:.2f}"))
            self.results_table.setItem(i, 9, QTableWidgetItem(str(prod_size)))
