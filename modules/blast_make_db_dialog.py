...
class MakeBlastDBThread(QThread):
    ...
    def run(self):
        exe = os.path.join(
            self.bin_dir, "makeblastdb.exe" if os.name == "nt" else "makeblastdb"
        )
        cmd = [
            exe,
            "-in",
...
class BlastMakeDbDialog(QDialog):
        ...
        if not bin_dir or not os.path.isdir(bin_dir):
            while True:
                ret = QMessageBox.question(
                    self,
                    "First Use",
                    "Please specify BLAST+ bin directory (contains makeblastdb, etc.)",
                ...
                dir_ = QFileDialog.getExistingDirectory(
                    self, "Choose BLAST+ bin directory"
                )
    ...
    def choose_outdir(self):
        dir_ = QFileDialog.getExistingDirectory(
            self, "Choose database output directory"
        )
        if dir_:
    ...
    def start_build(self):
        ...
        if not fasta or not os.path.isfile(fasta):
            QMessageBox.warning(
                self, "Input Error", "Please choose a valid FASTA file."
            )
            return
        if not outname:
            QMessageBox.warning(
                self, "Input Error", "Please enter an output database name."
            )
            return
        if not outdir or not os.path.isdir(outdir):
            QMessageBox.warning(
                self, "Input Error", "Please choose a database output directory."
            )
            return
        if not bin_dir or not os.path.isdir(bin_dir):
            QMessageBox.warning(
                self, "Configuration Error", "BLAST+ bin directory is not configured."
            )
            return
    ...
    def on_build_finished(self, success, msg):
        ...
        if success:
            QMessageBox.information(
                self, "Success", "Database built successfully.\n" + msg
            )
            self.accept()
        else:
