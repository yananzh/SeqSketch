...
class BlastMakeDbDialog(QDialog):
    def __init__(self, parent=None, status_callback=None):
        ...
        self.setWindowTitle("构建本地BLAST数据库")
        ...
        file_layout.addWidget(QLabel("输入FASTA文件:"))
        file_layout.addWidget(self.fasta_edit)
        file_layout.addWidget(file_btn)
        # 数据库类型
        type_layout = QHBoxLayout()
        self.prot_radio = QRadioButton("蛋白质 (prot)")
        self.nucl_radio = QRadioButton("核苷酸 (nucl)")
    ...
    def check_blast_bin(self):
        ...
        if not bin_dir or not os.path.isdir(bin_dir):
            while True:
                ret = QMessageBox.question(self, "首次使用", "首次使用，请指定BLAST+的bin目录（包含makeblastdb等）", QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel)
    ...
    def choose_fasta(self):
        file, _ = QFileDialog.getOpenFileName(self, "选择FASTA文件", "", "FASTA文件 (*.fasta *.fa *.faa *.txt);;所有文件 (*)")
        if file:
    ...
    def choose_outdir(self):
        dir_ = QFileDialog.getExistingDirectory(self, "选择数据库储存位置")
        if dir_:
    ...
    def start_build(self):
        ...
        if not fasta or not os.path.isfile(fasta):
            QMessageBox.warning(self, "输入错误", "请选择有效的FASTA文件！")
            return
        if not outname:
            QMessageBox.warning(self, "输入错误", "请填写输出数据库名称！")
            return
        if not outdir or not os.path.isdir(outdir):
            QMessageBox.warning(self, "输入错误", "请选择数据库储存位置！")
            return
        if not bin_dir or not os.path.isdir(bin_dir):
        ...
        if self.status_callback:
            self.status_callback("正在构建数据库，请稍候...")
    ...
    def on_build_finished(self, success, msg):
        ...
        if success:
            QMessageBox.information(self, "成功", "数据库构建成功！\n" + msg)
            self.accept()
        else:
            QMessageBox.critical(self, "失败", "数据库构建失败：\n" + msg) 