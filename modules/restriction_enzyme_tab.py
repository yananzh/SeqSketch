            ...
            for enz_name in self._enzyme_names:
                ...
                if positions:
                    cuts = (
                        [0] + positions + [seq_len]
                        if self._linear
                        else positions + [positions[0] + seq_len]
                    )
...
class RestrictionEnzymeTab(BaseTabWidget):
    ...
    def _draw_map(self, results, seq_len):
        ...
        for pos, enzymes in cut_map.items():
            ...
            ax.plot(
                [pos, pos], [-0.35, 0.35], color="#d32f2f", linewidth=1.5, alpha=alpha, zorder=1
            )
    ...
    def show_help(self):
        help_text = self.tr(
            ...
            "<h3>Quick Start</h3>"
            "<ol>"
            "<li>Select a FASTA file via <b>Browse</b> or drag-and-drop (file input only)</li>"
            "<li>Choose <b>Common</b> (commercial enzymes) or <b>All REBASE</b> (the restriction-enzyme database)</li>"
