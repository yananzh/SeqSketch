...
def _write_nexus_partition(
    ...
    gene_list = " ".join(gene for gene, _, _ in partitions)
    partition_name = "concat_partition"

    content = f"""#NEXUS
[ Generated by BioSeqAnalyzer – Sequence Concatenation & Partition ]
[ Sequence type: {seq_type} ]
[ Gene count: {len(partitions)} ]
[ Total length: {partitions[-1][2] if partitions else 0} positions ]

BEGIN TAXA;
    DIMENSIONS NTAX={len(set(g for g, _, _ in partitions))};
    TAXLABELS {" ".join(gene_list)};
END;

BEGIN SETS;
{chr(10).join(charset_lines)}
    partition {partition_name} = {len(partitions)}: {gene_list};
    SET partition = {partition_name};
END;
"""
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(content)
