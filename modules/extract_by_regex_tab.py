...
class ExtractByRegexWorker(QThread):
...
class ExtractByRegexTab(QWidget):
    def __init__(self):
        super().__init__()
        self._init_ui()
        self.worker = None
    def _init_ui(self):
        ...
        regex_layout.addWidget(QLabel("用于匹配ID的正则表达式:"))
        self.regex_edit = QLineEdit()
        regex_layout.addWidget(self.regex_edit)
        layout.addLayout(regex_layout)
        # 输出区
        output_layout = QHBoxLayout()
        output_layout.addWidget(QLabel("输出匹配的序列:"))
    ...
    def browse_input(self):
        ...
        if file_path:
            ...
            self.output_edit.setText(os.path.join(os.path.dirname(file_path), base + "_regex.fasta"))
    def browse_output(self):
        file_path, _ = QFileDialog.getSaveFileName(self, "保存匹配FASTA", "", "FASTA files (*.fasta *.fa *.fas);;All files (*)")
        if file_path:
    ...
    def run_extract(self):
        ...
        if not output_path:
            ...
            return
        if not regex:
            self.log.append("[错误] 正则表达式不能为空")
            return
        self.run_btn.setEnabled(False)
        self.progress.setText("处理中...")
        self.worker = ExtractByRegexWorker(input_path, regex, output_path)
