...
class BlastRunDialog(QDialog):
    def __init__(self, parent=None, get_query_seq=None, status_callback=None, result_callback=None):
        ...
        self.setWindowTitle(translations.tr("运行本地BLAST查询"))
        ...
        self.query_edit.setPlaceholderText(translations.tr("可粘贴FASTA序列或选择文件"))
        self.query_edit.setMinimumHeight(100)
        query_file_btn = QPushButton(translations.tr("选择文件"))
        query_file_btn.clicked.connect(self.choose_query_file)
        query_layout.addWidget(QLabel(translations.tr("查询序列:")))
        ...
        prog_layout.addWidget(QLabel(translations.tr("BLAST程序:")))
        prog_layout.addWidget(self.prog_combo)
        # 数据库
        db_layout = QHBoxLayout()
        self.db_edit = QLineEdit()
        self.db_edit.setReadOnly(True)
        db_btn = QPushButton(translations.tr("浏览..."))
        db_btn.clicked.connect(self.choose_db)
        db_layout.addWidget(QLabel(translations.tr("选择本地数据库:")))
        ...
        param_layout.addWidget(QLabel(translations.tr("线程数:")))
        param_layout.addWidget(self.threads_edit)
        param_layout.addWidget(QLabel("E-value:"))
        param_layout.addWidget(self.eval_edit)
        param_layout.addWidget(QLabel(translations.tr("NumOfHits:")))
        param_layout.addWidget(self.numhits_edit)
        # 输出文件
        out_layout = QHBoxLayout()
        self.out_edit = QLineEdit()
        self.out_edit.setPlaceholderText(translations.tr("请选择输出文件路径"))
        out_btn = QPushButton(translations.tr("选择输出文件"))
        out_btn.clicked.connect(self.choose_outfile)
        out_layout.addWidget(QLabel(translations.tr("输出文件:")))
        out_layout.addWidget(self.out_edit)
        out_layout.addWidget(out_btn)
        # 查询按钮
        self.run_btn = QPushButton(translations.tr("开始查询"))
    ...
    def check_blast_bin(self):
        ...
        if not bin_dir or not os.path.isdir(bin_dir):
            while True:
                ret = QMessageBox.question(self, translations.tr("首次使用"), translations.tr("首次使用，请指定BLAST+的bin目录（包含blastp等）"), QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel)
                ...
                if dir_ and os.path.isdir(dir_):
                    ...
                    break
    def choose_db(self):
        file, _ = QFileDialog.getOpenFileName(self, translations.tr("选择数据库主文件"), "", translations.tr("BLAST数据库主文件 (*)"))
        if file:
            self.db_edit.setText(os.path.splitext(file)[0])
    def choose_query_file(self):
        file, _ = QFileDialog.getOpenFileName(self, translations.tr("选择FASTA序列文件"), "", "FASTA文件 (*.fasta *.fa *.faa *.txt);;所有文件 (*)")
        if file:
            with open(file, 'r', encoding='utf-8') as f:
                seq = f.read()
            self.query_edit.setPlainText(seq)
    def choose_outfile(self):
        file, _ = QFileDialog.getSaveFileName(self, translations.tr("选择输出文件"), "blast_result.tsv", translations.tr("TSV文件 (*.tsv);;所有文件 (*)"))
        if file:
    ...
    def start_run(self):
        ...
        if not db:
            QMessageBox.warning(self, translations.tr("输入错误"), translations.tr("请选择本地数据库！"))
            return
        if not evalue:
            QMessageBox.warning(self, translations.tr("输入错误"), translations.tr("请填写E-value阈值！"))
            return
        if not num_threads.isdigit() or int(num_threads) < 1:
            QMessageBox.warning(self, translations.tr("输入错误"), translations.tr("线程数必须为正整数！"))
            return
        if not num_hits.isdigit() or int(num_hits) < 1:
            QMessageBox.warning(self, translations.tr("输入错误"), translations.tr("NumOfHits必须为正整数！"))
            return
        if not bin_dir or not os.path.isdir(bin_dir):
        ...
        if not query_seq or not query_seq.strip().startswith('>'):
            QMessageBox.warning(self, translations.tr("无效序列"), translations.tr("请输入一条有效的FASTA格式查询序列！"))
            return
        if not out_file:
            QMessageBox.warning(self, translations.tr("输出错误"), translations.tr("请选择输出文件路径！"))
            return
        self.run_btn.setEnabled(False)
        if self.status_callback:
            self.status_callback(translations.tr("正在运行BLAST查询，请稍候..."))
    ...
    def on_run_finished(self, success, out_file, msg):
        ...
        if success:
            QMessageBox.information(self, translations.tr("成功"), translations.tr("BLAST查询完成！结果已保存到：\n{path}").format(path=out_file))
            self.accept()
        else:
            QMessageBox.critical(self, translations.tr("失败"), translations.tr("BLAST查询失败：\n{msg}").format(msg=msg)) 