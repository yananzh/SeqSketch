...
class BlastResultTab(QWidget):
    def __init__(self, xml_path, parent=None):
        ...
        self.export_tsv_btn = QPushButton(translations.tr("导出TSV"))
        self.export_tsv_btn.clicked.connect(lambda: self.export_table('tsv'))
        self.export_html_btn = QPushButton(translations.tr("导出HTML"))
    ...
    def load_xml(self):
        try:
            ...
            QMessageBox.critical(self, translations.tr("解析错误"), translations.tr("BLAST XML解析失败: {error}").format(error=str(e)))
            return
        rows = []
    ...
    def export_table(self, fmt):
        if self.table.rowCount() == 0:
            QMessageBox.warning(self, translations.tr("无数据"), translations.tr("没有可导出的结果！"))
            return
        ext = fmt
        sep = ',' if fmt == 'csv' else '\t'
        file_path, _ = QFileDialog.getSaveFileName(self, translations.tr("导出为{format}").format(format=fmt.upper()), translations.tr("blast_result.{ext}").format(ext=ext), translations.tr("{format}文件 (*.{ext})").format(format=fmt.upper(), ext=ext))
        if file_path:
            ...
            QMessageBox.information(self, translations.tr("导出成功"), translations.tr("结果已导出: {path}").format(path=file_path))
    def export_html(self):
        if self.table.rowCount() == 0:
            QMessageBox.warning(self, translations.tr("无数据"), translations.tr("没有可导出的结果！"))
            return
        file_path, _ = QFileDialog.getSaveFileName(self, translations.tr("导出为HTML"), translations.tr("blast_result.html"), translations.tr("HTML文件 (*.html)"))
        if file_path:
